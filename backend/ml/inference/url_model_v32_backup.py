...
def detect_trusted_brand_impersonation(
    ...
    """
    Detect trusted-domain impersonation.
    ...
    registrable_domain = registrable_domain.lower().strip(".")
    ...
    for trusted_domain in trusted_domains:
        ...
        trusted_labels = trusted_domain.split(".")

        if len(trusted_labels) < 2:
            continue

        # Look for the trusted domain as consecutive
        # hostname labels.
        for i in range(
            len(labels) - len(trusted_labels) + 1
        ):
            candidate = ".".join(
                labels[
                    i:i + len(trusted_labels)
                ]
            )

            if candidate == trusted_domain:

                # Legitimate domain/subdomain.
                if registrable_domain == trusted_domain:
                    return None
...
class URLMLModel:
    ...
    def predict(
        ...
        # A trusted registrable domain gets a benign
        # override only when the URL itself does not
        # contain strong deception indicators.
        #
        # This prevents:
        #
        # google.com.evil.xyz
        ...
        # from being trusted.
        #
        # It also avoids overriding URLs containing:
        #
        # IPs
        # @
        # punycode
