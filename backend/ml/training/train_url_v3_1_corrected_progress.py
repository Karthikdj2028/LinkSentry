...
class StageHeartbeat:
    """Print a heartbeat during long sklearn operations."""

    def __init__(self, stage, interval=15):
        self.stage = stage
...
def extract_features_with_progress(urls, stage, report_every=10000):
    """Extract URL features with progress, speed, elapsed time and ETA."""
    urls = list(urls)
    total = len(urls)
...
TOP_DOMAIN_HARD_NEGATIVE_DATASET = os.path.join(
    DATA_DIR,
...
REAL_BENCHMARK_OUTPUT = os.path.join(
    DATA_DIR,
    "v3_1_unseen_benign_benchmark.csv"
)
...
print("\nLoading V3.1 hard-negative dataset...")

hard_df = pd.read_csv(
    HARD_NEGATIVE_DATASET,
    usecols=["url", "type"]
)
print("\nLoading V3.1 top-domain hard negatives...")

top_domain_df = pd.read_csv(
    TOP_DOMAIN_HARD_NEGATIVE_DATASET
)

top_domain_df = top_domain_df.dropna(
    subset=["url", "type"]
)
...
top_domain_df = top_domain_df[
    top_domain_df["url"] != ""
]

top_domain_df = top_domain_df.drop_duplicates(
    subset=["url"]
)

print(
    "Top-domain hard negatives:",
    len(top_domain_df)
)
...
top_domain_train = sample_class(
    top_domain_df,
    "benign_top_domain",
    TARGETS["benign_top_domain"],
)
...
print(
    f"\nTotal V3.1 training candidates: "
    f"{len(training_df):,}"
)
...
# Also exclude domains used by V3.1 top-domain hard negatives.
# This prevents benchmark leakage from the 50,000 top-domain
# hard-negative training URLs.
top_domain_domains = set(
    top_domain_df["url"].map(extract_domain)
)
...
X_train_struct = extract_features_with_progress(
    train_df["url"],
    "TRAIN STRUCTURAL FEATURES",
)

X_val_struct = extract_features_with_progress(
    validation_df["url"],
    "VALIDATION STRUCTURAL FEATURES",
)

X_test_struct = extract_features_with_progress(
    test_df["url"],
...
print(
    f"Training URLs: {len(train_df):,} | "
    f"Validation URLs: {len(validation_df):,} | "
...
with StageHeartbeat("TF-IDF fit + train transform"):
    X_train_tfidf = vectorizer.fit_transform(
        train_df["url"]
    )

with StageHeartbeat("TF-IDF validation transform"):
    X_val_tfidf = vectorizer.transform(
        validation_df["url"]
    )

with StageHeartbeat("TF-IDF test transform"):
...
with StageHeartbeat("LinearSVC training"):
    classifier.fit(
        X_train,
        y_train
    )
...
X_real_struct = extract_features_with_progress(
    benchmark_df["url"],
    "REAL BENCHMARK STRUCTURAL FEATURES",
)

with StageHeartbeat("REAL BENCHMARK TF-IDF transform"):
...
with StageHeartbeat("REAL BENCHMARK prediction"):
...
model_package = {
    ...
    "model_type": (
        ...
        "LinearSVC + hard-negative training + "
        "top-domain hard negatives"
    ),
...
print("\nV3.1 training completed successfully.")
